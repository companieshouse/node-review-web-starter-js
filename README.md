...
_**If you're having `eslint` errors during installation, try running this command instead:**_
...
- A similar approach to this is router helpers which contain routines and methods that you'd rather see obscured from the primary router file. Router helpers, if desired, can be created as a sub-folder within the main router folder, however, it might be sufficient to add all your helpers to the generic handler (`./src/routers/handlers/generic.js`)  - which exposes common methods to all handlers - without the need to use the "utilities" approach.
...
- [x] Resolve the `ESLINT_LEGACY_OBJECT_REST_SPREAD` deprecation warning when linting
